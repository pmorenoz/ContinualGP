...
class ContinualGP(GPy.core.SparseGP):
    def __init__(self, X, Y, Z, kern_list, likelihood, Y_metadata, name='ContinualGP', batch_size=None, Zold=None, kern_list_old=None, W_list=None):
        ...
        if kern_list_old is None:
            self.kern_list_old = kern_list
        else:
            self.kern_list_old = kern_list_old

        self.num_inducing_old = self.Zold.shape[0] # Mold
        ...
        # NEW ----> OLD
        self.phi_means = Param('phi_means', 0*np.random.randn(self.num_inducing_old, self.num_latent_funcs) +
                               0*np.tile(np.random.randn(1,self.num_latent_funcs),(self.num_inducing_old,1)))
        phi_old_chols = choleskies.triang_to_flat(np.tile(np.eye(self.num_inducing_old)[None,:,:], (self.num_latent_funcs,1,1)))
        self.phi_chols = Param('phi_chols', phi_old_chols)
